...
function BarcodeCanvas({ value, width=240, height=55 }) {
  ...
  useEffect(() => {
    ...
    // Generate bars from value chars
    const bars = [3,1,1];
      ...
      <div style={{display:'flex',gap:8,marginBottom:'1rem',flexWrap:'wrap',alignItems:'center'}}>
        {user?.role==='admin' && (
          <>
            ...
            {shops.map(s=><button key={s.id} className={`filter-tab ${shopId==s.id?'active':''}`} onClick={()=>setShopId(s.id)}>{s.name}</button>)}
            <span style={{flex:1}}/>
          </>
        )}
      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(210px,1fr))',gap:12}}>
        {products.map(p=>(
          <div key={p.id} className="card" style={{padding:'1rem'}}>
            <div style={{textAlign:'center',fontSize:34,marginBottom:8,background:'var(--surface)',borderRadius:8,padding:'10px 0'}}>{EMOJI[p.category]||'📦'}</div>
            ...
            <div style={{display:'flex',gap:4,marginBottom:8,flexWrap:'wrap'}}>
              ...
              <span className={`badge ${p.stock>0?'badge-green':'badge-red'}`}>{p.stock>0?`${t.inStock}: ${p.stock}`:t.outOfStock}</span>
            </div>
            ...
            <div style={{display:'flex',gap:6}}>
              <button className="btn btn-outline btn-sm" style={{flex:1}} onClick={()=>openEdit(p)}><i className="ti ti-edit"/>{t.editProduct}</button>
              {user?.role==='admin' && <button className="btn btn-sm" style={{background:'rgba(231,76,60,.15)',color:'var(--danger)',border:'none'}} onClick={()=>handleDelete(p.id)}><i className="ti ti-trash"/></button>}
            </div>
          </div>
        ))}
        {!products.length && <div className="empty-state" style={{gridColumn:'1/-1'}}><i className="ti ti-package-off"/>No products</div>}
      </div>
        ...
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,.7)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:100,padding:'1rem'}}>
          <div style={{background:'var(--surface)',border:'1px solid var(--border)',borderRadius:16,width:'100%',maxWidth:500}}>
            ...
            <form onSubmit={handleSave} style={{padding:'1.5rem'}}>
              <div className="form-group"><label className="form-label">{t.productName}</label><input className="form-control" value={form.name} onChange={e=>setForm(f=>({...f,name:e.target.value}))} required /></div>
              ...
              <div className="form-group"><label className="form-label">{t.compatible}</label><input className="form-control" value={form.compatible} onChange={e=>setForm(f=>({...f,compatible:e.target.value}))} /></div>
              <div className="form-row3">
                <div className="form-group"><label className="form-label">{t.costPrice} €</label><input className="form-control" type="number" value={form.cost_price} onChange={e=>setForm(f=>({...f,cost_price:e.target.value}))} required /></div>
                <div className="form-group"><label className="form-label">{t.sellPrice} €</label><input className="form-control" type="number" value={form.sell_price} onChange={e=>setForm(f=>({...f,sell_price:e.target.value}))} required /></div>
                <div className="form-group"><label className="form-label">{t.stockQty}</label><input className="form-control" type="number" value={form.stock} onChange={e=>setForm(f=>({...f,stock:e.target.value}))} required /></div>
              </div>
